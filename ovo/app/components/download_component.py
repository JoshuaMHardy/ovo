...
from ovo import db, storage, Pool
...
@st.fragment
def download_job_designs_component(design_ids: list[str], pools: list[Pool], key: str = "default"):
    """
...
def download_descriptor_table(filename, design_ids, descriptor_keys=None, key="default", width="content"):
    if st.button("Download descriptor table", key=f"prepare_descriptors_{key}", width=width):
        with st.spinner("Preparing descriptor table..."):
            excel_bytes = export_design_descriptors_excel(design_ids, descriptor_keys=descriptor_keys)
